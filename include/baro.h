...
void baro_update(){
    ...
    altitude = kalman_filter(altitude); //dinyalain soalny tuning pake kalman sm lpf
    uint32_t ttnow = millis();
    
    float delta_alt = altitude - _last_alt;
    vertical_velocity = delta_alt * (ttnow - tt1);
    tt1 = ttnow;
    _last_alt = altitude;
...
void baro_init(){
    ms5611_Init();
    lowpass_init(&pressure_lpf, 100, 50);
    lowpass_init(&temperature_lpf, 100, 50);
    // readRaw();
    // CalculateTemperature();
    // update_raw();
    // threads.addThread(readRawTemperature);
    threads.addThread(baro_update);
    delay(100);
    readPressure();
    new_sea_pressure = getSeaLevel(pressure, 0.6);
    // Serial.print(new_sea_pressure);
    // delay(1000);
}
