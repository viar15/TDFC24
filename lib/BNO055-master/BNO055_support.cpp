...
{
	BNO055_RETURN_FUNCTION_TYPE comres = BNO055_Zero_U8X;
	Wire2.beginTransmission(dev_addr);	//Start of transmission
	Wire2.write(reg_addr);				//Desired start register
	Wire2.endTransmission();	//Stop of transmission
	delayMicroseconds(150);
	Wire2.requestFrom(dev_addr, cnt);	//Request data
	while(Wire2.available())				//The slave device may send less than requested
	{
		*reg_data = Wire2.read();		//Receive a byte
...
BNO055_RETURN_FUNCTION_TYPE BNO055_I2C_bus_write(unsigned char dev_addr,unsigned char reg_addr, unsigned char *reg_data, unsigned char cnt)
{
	...
	{
		Wire2.write(*reg_data);			//Write the data
		reg_data++;						//Increment pointer
	}
	Wire2.endTransmission();	//Stop of transmission
	delayMicroseconds(150);
